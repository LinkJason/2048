...
class Game2048 {
    ...
    private scoreSpan: JQuery<HTMLElement>; //分数
    private mainPanel: JQuery<HTMLElement>; //主面板
    private numBoxes: NumBox[] = new Array(16).fill(null); //16个空位的存放情况
    ...
    private initUi() {
        ...
        this.container.css("width", width); //宽度更新
        //分数显示
        const uiPanel = $(`
    ...
    private bindKeys() {
        const mergeQueue = [];
        const addMerge = merge => { //创建缓冲区改善按键响应丢失问题
    ...
    private async merge(startIndexes: number[], nextDelta: number) {
        ...
        if ((this.isGameOver = this.checkGameOver())) {//显示游戏结束
            setTimeout(() => {
                alert("Game Over!");
            }, 500);
        }
